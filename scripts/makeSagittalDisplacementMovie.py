...
class ImageGenerator(object):
    ...
    def generate_image(self, time_idx, view="x"):
        ...
        arrows_d = clipped_data_d.glyph(scale=d, factor=20, orient=d)
        p.add_mesh(arrows_d, scalars='GlyphScale', cmap ="amp", clim=self.d_range,
                   stitle="disp Magnitude [m]", scalar_bar_args = scalar_bars["left"])
        p.add_mesh(clipped_data_d, color="red", opacity=0.5) 

        arrows_u = clipped_data_u.glyph(scale=u, factor=1, orient=u)
        p.add_mesh(arrows_u, scalars='GlyphScale', cmap ="speed", clim=self.u_range,
